...
describe('RFQ Builder (e2e)', () => {
  ...
  async function mkVendor(name: string, status: string): Promise<string> {
    const v = await prisma.vendor.create({
      data: {
        ...
        contactEmail: `${name.replace(/\W/g, '')}@y.com`,
  ...
  beforeAll(async () => {
    ...
    const scm = await prisma.vertical.findUniqueOrThrow({
      where: { code: 'SCM' },
    });
    ...
    pmId = pm.id;
    await http()
      .patch(`/employees/${pm.id}/designate-project-manager`)
      .set('Authorization', `Bearer ${superAdminToken}`)
      .expect(200);
  ...
  it('rejects issuing with fewer than 3 invitees', async () => {
    ...
    await http()
      .post(`/rfqs/${rfq.id}/invitees`)
      .set('Authorization', `Bearer ${scmManagerToken}`)
      .send({ vendorId: vendorIds[0] })
      .expect(201);
    await http()
      .post(`/rfqs/${rfq.id}/invitees`)
      .set('Authorization', `Bearer ${scmManagerToken}`)
      .send({ vendorId: vendorIds[1] })
      .expect(201);
    // 2 invitees → issue rejected.
    await http()
      .post(`/rfqs/${rfq.id}/issue`)
      .set('Authorization', `Bearer ${scmManagerToken}`)
      .expect(400);
  ...
  it('warns (not blocks) when inviting an unqualified vendor and snapshots its status', async () => {
    ...
    const res = await http()
      ...
      .set('Authorization', `Bearer ${scmManagerToken}`)
      .send({ vendorId: vendorIds[2] })
      .expect(201); // PENDING_QUESTIONNAIRE
    expect(res.body.data.qualificationWarning).toMatch(/not qualified/i);
    const inv = res.body.data.rfq.invitees.find(
      (i: any) => i.vendorId === vendorIds[2],
    );
    ...
    await prisma.vendor.update({
      where: { id: vendorIds[2] },
      data: { status: 'PENDING_QUESTIONNAIRE' },
    });
  });
  ...
  it('SEALED BIDS: quote values are not retrievable server-side while ISSUED', async () => {
    ...
    await http()
      .post(`/public/rfq-quote/${tokens[0]}/submit`)
      .send({
        lines: [
  ...
  it('supports save-and-resume, decline, and shows non-responders in the comparison', async () => {
    ...
    const saved = (
      await http()
        .post(`/public/rfq-quote/${t[0]}/save`)
        .send({ lines: [{ rfqLineId: l0, unitPrice: 12 }] })
        .expect(201)
    ).body.data;
    expect(saved.quote.lines).toHaveLength(1);
    await http()
      .post(`/public/rfq-quote/${t[0]}/submit`)
      .send({
        lines: [
          { rfqLineId: l0, unitPrice: 12 },
          { rfqLineId: l1, unitPrice: 6 },
        ],
    ...
    await http()
      .post(`/public/rfq-quote/${t[1]}/submit`)
      .send({
        lines: [
          { rfqLineId: l0, unitPrice: 9 },
          { rfqLineId: l1, unitPrice: 5 },
        ],
    ...
    // Invitee 2: decline.
    await http()
      .post(`/public/rfq-quote/${t[2]}/decline`)
      .send({ declineReason: 'Capacity full' })
      .expect(201);
    ...
    await http()
      ...
      .send({
        lines: [
          { rfqLineId: l0, unitPrice: 1 },
          { rfqLineId: l1, unitPrice: 1 },
        ],
      })
      .expect(403);
    ...
    expect(
      cmp.columns.filter((c: any) => c.weightedScore !== null),
    ).toHaveLength(2);
  });
  ...
  it('award: lowest needs no justification; non-lowest requires it; PM-gated; pre-fills a DRAFT PO', async () => {
    ...
    const awarded = (
      await http()
        ...
        .send({
          inviteeId: invId[0],
          justification: 'Better lead time and prior quality record',
        })
        .expect(201)
    ).body.data;
  ...
  it('generates a DRAFT RFQ from a kickoff shortfall, linked to the kickoff', async () => {
    ...
    const sup = await prisma.supplier.create({
      data: {
        ...
        contactPersonName: 'x',
        contactPersonDesignation: 'x',
        contactEmail: `rfqsup${suffix.replace(/\W/g, '')}@y.com`,
        contactPhone: '+910000000000',
        createdById: superAdminId,
        status: 'APPROVED',
      },
    ...
    await prisma.itemSupplier.create({
      data: { itemId: raw.id, supplierId: sup.id, createdById: superAdminId },
    });
    ...
    await prisma.orderConfirmationSheet.create({
      data: {
        orderId: order.id,
        confirmationNumber: `OC-RFQ-${suffix}`,
        revisionNumber: 1,
        status: 'EXECUTED',
        createdById: superAdminId,
    ...
    await http()
      .post(`/rfqs/from-kickoff/${kickoff.id}`)
      .set('Authorization', `Bearer ${scmManagerToken}`)
      .expect(400);
    ...
    const store = await prisma.storeLocation.findFirstOrThrow({
      where: { code: 'MAIN' },
    });
    await prisma.stockBalance.create({
      data: { itemId: raw.id, storeLocationId: store.id, onHandQuantity: '5' },
    });
