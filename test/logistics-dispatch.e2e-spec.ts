...
describe('Logistics & Dispatch (e2e)', () => {
  ...
  async function seedOrder(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const product = await prisma.product.create({
        data: {
          sku: `DISP-SKU-${suffix}-${i}`,
          name: `Widget ${i}`,
          unitPrice: String(lines[i].unitPrice),
          unitOfMeasure: 'NOS',
          hsnCode: `8479${i}`,
          itemId: item.id,
        },
      ...
      lineData.push({
        ...
        unitPrice: String(lines[i].unitPrice),
        lineTotal: String(lines[i].qty * lines[i].unitPrice),
      });
      // Seed on-hand via the ledger directly.
      const bal = await prisma.stockBalance.upsert({
        where: {
          itemId_storeLocationId: { itemId: item.id, storeLocationId: storeId },
        },
        create: {
          itemId: item.id,
          storeLocationId: storeId,
          onHandQuantity: String(lines[i].onHand),
        },
  ...
  beforeAll(async () => {
    ...
    await http()
      .patch(`/employees/${qc.id}/designate-qc-inspector`)
      .set('Authorization', `Bearer ${superAdminToken}`)
      .expect(200);
  ...
  it('blocks dispatch until final QC is cleared, and only a QC inspector can clear it', async () => {
    ...
    const dc = (
      await http()
        .post('/logistics/delivery-challans')
        .set('Authorization', `Bearer ${prodToken}`)
        .send({
          orderId: o.orderId,
          consigneeName: 'ACME',
          consigneeAddress: 'Blr',
          consigneeStateCode: '29',
          transportMode: 'ROAD',
          lines: [{ orderLineId: o.lineIds[0], quantity: 10 }],
        })
        .expect(201)
    ).body.data;
    ...
    await http()
      .post(`/logistics/delivery-challans/${dc.id}/dispatch`)
      .set('Authorization', `Bearer ${prodToken}`)
      .expect(400);
  ...
  it('a logistics (non-finance) user dispatches; a DRAFT invoice is created (never ISSUED, no journal), STOCK_OUT generated, place-of-supply + HSN seeded', async () => {
    ...
    const dc = (
      await http()
        ...
        .send({
          ...
          transportMode: 'ROAD',
          lines: [{ orderLineId: o.lineIds[0], quantity: 4 }],
        })
        .expect(201)
    ).body.data;
    ...
    const adj = await prisma.stockAdjustment.findFirst({
      where: { itemId: o.itemIds[0], reason: { contains: dc.dcNumber } },
    });
    ...
    const inv = await prisma.salesInvoice.findUniqueOrThrow({
      where: { id: dispatched.linkedInvoiceId },
      include: { lines: true },
    });
  ...
  it('gates dispatch to Production/SA — a Sales user cannot create a DC', async () => {
    ...
    await http()
      ...
      .send({
        ...
        consigneeStateCode: '29',
        transportMode: 'ROAD',
        lines: [{ orderLineId: o.lineIds[0], quantity: 1 }],
      })
      .expect(403);
    // But sales CAN read the register (company-wide read).
    await http()
      .get('/logistics/delivery-challans')
      .set('Authorization', `Bearer ${salesToken}`)
      .expect(200);
  });
  ...
  it('supports partial dispatch across multiple DCs with correct previously-dispatched + derived fulfilment status; each DC gets its own draft invoice', async () => {
    ...
    const dc1 = (
      await http()
        ...
        .send({
          ...
          consigneeStateCode: '29',
          transportMode: 'ROAD',
          lines: [{ orderLineId: o.lineIds[0], quantity: 6 }],
        })
        .expect(201)
    ).body.data;
    const disp1 = (
      await http()
        .post(`/logistics/delivery-challans/${dc1.id}/dispatch`)
        .set('Authorization', `Bearer ${prodToken}`)
        .expect(201)
    ).body.data;
    ...
    const disp2 = (
      await http()
        .post(`/logistics/delivery-challans/${dc2.id}/dispatch`)
        .set('Authorization', `Bearer ${prodToken}`)
        .expect(201)
    ).body.data;
    ...
    const dc = (
      await http()
        ...
        .send({
          ...
          consigneeStateCode: '29',
          transportMode: 'ROAD',
          lines: [{ orderLineId: o.lineIds[0], quantity: 8 }],
        })
        .expect(201)
    ).body.data;
    const disp = (
  ...
  it('computes the OTD report (on-time and late cases)', async () => {
    ...
    const dcA = (
      await http()
        ...
        .send({
          ...
          consigneeStateCode: '29',
          transportMode: 'ROAD',
          promisedDeliveryDate: '2026-06-10T00:00:00.000Z',
          lines: [{ orderLineId: o1.lineIds[0], quantity: 2 }],
        })
        .expect(201)
    ).body.data;
    await http()
      .post(`/logistics/delivery-challans/${dcA.id}/dispatch`)
      .set('Authorization', `Bearer ${prodToken}`)
      .expect(201);
    ...
    await prisma.deliveryChallan.update({
      ...
      data: {
        status: 'DELIVERED',
        actualDeliveryDate: new Date('2026-06-08T00:00:00.000Z'),
      },
    });
    ...
    const dcB = (
      await http()
        ...
        .send({
          ...
          transportMode: 'ROAD',
          promisedDeliveryDate: '2026-06-10T00:00:00.000Z',
          lines: [{ orderLineId: o2.lineIds[0], quantity: 2 }],
        })
        .expect(201)
    ).body.data;
    await http()
      .post(`/logistics/delivery-challans/${dcB.id}/dispatch`)
      .set('Authorization', `Bearer ${prodToken}`)
      .expect(201);
    await prisma.deliveryChallan.update({
      where: { id: dcB.id },
      data: {
        status: 'DELIVERED',
        actualDeliveryDate: new Date('2026-06-15T00:00:00.000Z'),
      },
    });
    ...
    const otd = (
      await http()
        .get(
          '/logistics/otd?from=2026-06-01T00:00:00.000Z&to=2026-06-30T00:00:00.000Z',
        )
        .set('Authorization', `Bearer ${prodToken}`)
        .expect(200)
    ).body.data;
    ...
    const lateRow = otd.dispatches.find(
      (d: any) => d.dcNumber === dcB.dcNumber,
    );
    expect(lateRow.onTime).toBe(false);
    expect(lateRow.delayDays).toBe(5);
    const onTimeRow = otd.dispatches.find(
      (d: any) => d.dcNumber === dcA.dcNumber,
    );
