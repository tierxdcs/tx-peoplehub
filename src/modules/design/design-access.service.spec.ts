...
describe('DesignAccessService', () => {
  const prisma: any = { employee: { findUnique: jest.fn() } },
    service = new DesignAccessService(prisma),
    user: any = { id: 'e1', role: Role.EMPLOYEE };
  beforeEach(() => jest.clearAllMocks());
  it('allows active Design and R&D vertical users to prepare designs', async () => {
    prisma.employee.findUnique.mockResolvedValue({
      status: 'ACTIVE',
      isDesignHead: false,
      vertical: { code: 'DESIGN' },
    });
  ...
  it('allows only the explicit Design Head to release', async () => {
    prisma.employee.findUnique.mockResolvedValue({
      status: 'ACTIVE',
      isDesignHead: true,
      vertical: null,
  ...
  it('does not give Super Admin implicit release authority', async () => {
    prisma.employee.findUnique.mockResolvedValue({
      status: 'ACTIVE',
      isDesignHead: false,
      vertical: null,
  ...
  it('rejects inactive capability holders', async () => {
    prisma.employee.findUnique.mockResolvedValue({
      status: 'INACTIVE',
      isDesignHead: true,
      vertical: { code: 'DESIGN' },
