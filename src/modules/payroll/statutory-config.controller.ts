...
export class StatutoryConfigController {
  ...
  @Patch(':id')
  @Roles(Role.ADMIN, Role.SUPER_ADMIN, Role.MANAGER)
  @ApiOperation({ summary: 'Edit one statutory config version' })
  update(@Param('id') id: string, @Body() dto: UpdateStatutoryConfigDto) {
    return this.statutoryConfigService.update(id, dto);
  }
