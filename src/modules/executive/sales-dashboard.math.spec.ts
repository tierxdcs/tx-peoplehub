...
describe('samePeriodLastYear', () => {
  it('truncates the prior year to the same number of elapsed months', () => {
    ...
    const window = samePeriodLastYear(fiscalYearFor(utc(2026, 8, 25)), utc(2026, 8, 25));
  ...
  it('covers the full prior year once the current year is complete', () => {
    const window = samePeriodLastYear(fiscalYearFor(utc(2027, 3, 31)), utc(2027, 3, 31));
...
describe('monthsToDate', () => {
  it('runs from April to the month containing now, inclusive', () => {
    const buckets = monthsToDate(fiscalYearFor(utc(2026, 8, 25)), utc(2026, 8, 25));
  ...
  it('never runs past March of the closing year', () => {
    const buckets = monthsToDate(fiscalYearFor(utc(2027, 3, 31)), utc(2027, 3, 31));
    expect(buckets).toHaveLength(12);
    expect(buckets[11].key).toBe('2027-03');
  });

  it('yields a single bucket in the first month of the year', () => {
    expect(monthsToDate(fiscalYearFor(utc(2026, 4, 2)), utc(2026, 4, 2))).toHaveLength(1);
  });

  it('gives each bucket a half-open range that matches monthKeyOf', () => {
    const [april] = monthsToDate(fiscalYearFor(utc(2026, 4, 2)), utc(2026, 4, 2));
    expect(monthKeyOf(april.startsOn)).toBe('2026-04');
    expect(monthKeyOf(new Date(april.endsBefore.getTime() - 1))).toBe('2026-04');
...
describe('daysBetween', () => {
  ...
  it('floors a partial day', () => {
    expect(
      daysBetween(new Date(Date.UTC(2026, 3, 1, 23)), new Date(Date.UTC(2026, 3, 2, 1))),
    ).toBe(0);
  });
});
...
describe('shares', () => {
  ...
  it('reports null percent against a zero total', () => {
    expect(shares([{ name: 'A', value: d(0) }], d(0))[0].percentOfTotal).toBeNull();
  });

  it('carries extra fields through so a slice keeps its colour', () => {
    const result = shares([{ name: 'A', value: d(1), colorHex: '#111111' }], d(1));
