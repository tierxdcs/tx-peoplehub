...
describe('QmsService — order-line linking', () => {
  ...
  describe('COPQ disposition', () => {
    it('calculates Scrap from affected quantity and realized item cost', async () => {
      prisma.qmsNonConformance.findUnique.mockResolvedValue({
        id: 'n-1',
        status: 'CONTAINED',
        itemId: 'item-1',
        affectedQuantity: 3,
      ...
      itemCosts.currentFailureCost.mockResolvedValue({
        amount: { mul: (q: any) => Number(q) * 125 },
        source: 'LATEST_ACCEPTED_GRN',
      });
      await service.dispositionNcr(
        'n-1',
        { disposition: 'SCRAP' } as never,
        user,
      );
      expect(prisma.qmsNonConformance.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            costOfPoorQuality: 375,
            costOfPoorQualitySource: 'SYSTEM_CALCULATED',
          }),
        }),
      );
    });
    ...
    it('does not invent a cost for Rework', async () => {
      ...
      expect(prisma.qmsNonConformance.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            costOfPoorQuality: null,
            costOfPoorQualitySource: null,
          }),
        }),
      );
    });
    ...
    it('marks an entered override as manual', async () => {
      ...
      await service.dispositionNcr(
        'n-1',
        { disposition: 'SCRAP', costOfPoorQuality: 280 } as never,
        user,
      );
      expect(itemCosts.currentFailureCost).not.toHaveBeenCalled();
      expect(prisma.qmsNonConformance.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ costOfPoorQualitySource: 'MANUAL' }),
        }),
      );
    });
  });
