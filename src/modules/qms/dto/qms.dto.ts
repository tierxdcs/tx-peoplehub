...
export class QmsQuestionDto {
  ...
  @ApiProperty({ enum: QmsResponseType })
  @IsEnum(QmsResponseType)
...
export class CreateQmsTemplateDto {
  ...
  @ApiProperty({ enum: QmsTemplateType })
  @IsEnum(QmsTemplateType)
  templateType!: QmsTemplateType;
  @ApiProperty({ type: [QmsQuestionDto] })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => QmsQuestionDto)
  questions!: QmsQuestionDto[];
}
export class QmsPlanStageDto {
  @IsString() name!: string;
  @IsString() stageCode!: string;
  @Type(() => Number) @IsNumber() sequence!: number;
  @IsEnum(QmsControlPoint) controlPoint!: QmsControlPoint;
  @IsString() templateId!: string;
  @IsOptional() @IsBoolean() customerWitnessRequired?: boolean;
  @IsOptional() @IsBoolean() blocksNextStage?: boolean;
...
export class CompleteQmsInspectionDto {
  ...
  @ValidateNested({ each: true })
  @Type(() => QmsResponseDto)
  responses!: QmsResponseDto[];
  @IsEnum(QmsInspectionResult) overallResult!: QmsInspectionResult;
  @IsOptional() @Type(() => Number) @IsNumber() quantityInspected?: number;
  @IsOptional() @Type(() => Number) @IsNumber() quantityAccepted?: number;
  @IsOptional() @Type(() => Number) @IsNumber() quantityRejected?: number;
