...
describe('FinanceAccessService', () => {
  ...
  const prisma: any = {
    ...
    financeAuditorGrant: { findUnique: jest.fn() },
  };
  ...
  it('gives Accounts-vertical staff operational finance access without approval', async () => {
    prisma.employee.findUnique.mockResolvedValue({
      ...
      vertical: { code: 'ACCOUNTS' },
    });
    await expect(service.accessFor(user)).resolves.toEqual({
      isFinanceUser: true,
      isAccountsHead: false,
      isFinanceAuditor: false,
    });
    await expect(service.assertCanUseFinance(user)).resolves.toBeUndefined();
    await expect(service.assertAccountsHead(user)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
  ...
  it('gives an active Super Admin full finance approval authority', async () => {
    ...
    await expect(service.accessFor(superAdmin)).resolves.toEqual({
      isFinanceUser: true,
      isAccountsHead: true,
      isFinanceAuditor: false,
    });
    await expect(
      service.assertCanUseFinance(superAdmin),
    ).resolves.toBeUndefined();
    await expect(
      service.assertAccountsHead(superAdmin),
    ).resolves.toBeUndefined();
  });
  ...
  it('allows an active auditor grant to view reports without mutation access', async () => {
    ...
    prisma.financeAuditorGrant.findUnique.mockResolvedValue({
      isActive: true,
      expiresAt: null,
    });
    await expect(service.assertCanViewFinance(user)).resolves.toBeUndefined();
    await expect(service.assertCanUseFinance(user)).rejects.toBeInstanceOf(
      ForbiddenException,
    );
  });
