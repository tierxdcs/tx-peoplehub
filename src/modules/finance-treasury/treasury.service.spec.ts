...
describe('TreasuryService accounting controls', () => {
  ...
  const prisma = {
    fxRevaluationRun: { findUnique: jest.fn(), update: jest.fn() },
  };
  ...
  it('recognizes an AR gain when the closing rate rises', () => {
    const line = (service as any).fxLine(
      'AR',
      'i1',
      'INV-1',
      'Customer',
  ...
  it('prevents self-approval of an FX run', async () => {
    prisma.fxRevaluationRun.findUnique.mockResolvedValue({
      id: 'fx-1',
    ...
    await expect(
      service.approveFx('fx-1', { id: 'head-1' } as any),
    ).rejects.toThrow('cannot approve an FX run they created');
  });
});
