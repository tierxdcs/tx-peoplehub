...
export class ProvisioningService {
  ...
  async createForEmployee(
    ...
    const items = await db.provisioningItemType.findMany({
      where: { isActive: true },
      select: { id: true },
    });
  ...
  listItemTypes(user: AuthenticatedUser) {
    ...
    return this.prisma.provisioningItemType.findMany({
      include: {
        approverVertical: {
          select: { id: true, name: true, code: true, ownerId: true },
        },
  ...
  async createItemType(
    dto: CreateProvisioningItemTypeDto,
  ...
  async updateItemType(
    id: string,
    dto: UpdateProvisioningItemTypeDto,
    user: AuthenticatedUser,
  ) {
    this.assertSuperAdmin(user);
    const current = await this.prisma.provisioningItemType.findUnique({
      where: { id },
    });
    if (!current)
      throw new NotFoundException('Provisioning item type not found');
    const data = await this.normalizeItemType({ ...current, ...dto });
    return this.prisma.provisioningItemType.update({
      where: { id },
      data: { ...data, isActive: dto.isActive ?? current.isActive },
    });
  }
  ...
  async listForEmployee(employeeId: string, user: AuthenticatedUser) {
    if (
      ...
      const viewer = await this.prisma.employee.findUnique({
        where: { id: user.id },
        select: { role: true, vertical: { select: { code: true } } },
      });
      if (viewer?.role !== Role.MANAGER || viewer.vertical?.code !== 'HR') {
        throw new ForbiddenException(
          'Only HR Managers, Admins, or the employee may view this provisioning checklist',
        );
      }
    }
  ...
  /** Badge summary: how many requests await this approver, and when the oldest
   *  was raised. Same where-clause and oldest-first ordering as listPending. */
  async pendingApprovalQueue(user: AuthenticatedUser): Promise<PendingQueue> {
    ...
    const [count, oldest] = await Promise.all([
      this.prisma.provisioningRequest.count({ where }),
      this.prisma.provisioningRequest.findFirst({
        where,
        orderBy: { createdAt: 'asc' },
  ...
  async approve(id: string, user: AuthenticatedUser) {
    ...
    if (request.status !== ProvisioningRequestStatus.PENDING_APPROVAL)
    ...
    return this.prisma.provisioningRequest.update({
      ...
      data: physical
        ? {
            status: ProvisioningRequestStatus.SENT_TO_SCM,
  ...
  async listScmQueue(user: AuthenticatedUser) {
    ...
    return this.prisma.provisioningRequest.findMany({
      where: { status: ProvisioningRequestStatus.SENT_TO_SCM },
      include: requestInclude,
  ...
  async fulfill(id: string, user: AuthenticatedUser) {
    ...
    const request = await this.findRequest(id);
    if (
      request.status !== ProvisioningRequestStatus.SENT_TO_SCM ||
      !request.itemType.requiresScmFulfillment
    )
      throw new BadRequestException(
        'Only SCM-bound physical requests can be fulfilled',
      );
    return this.prisma.provisioningRequest.update({
      where: { id },
      data: {
        status: ProvisioningRequestStatus.FULFILLED,
  ...
  private async findRequest(id: string) {
    const request = await this.prisma.provisioningRequest.findUnique({
      where: { id },
      include: requestInclude,
    });
  ...
  private approverWhere(
    user: AuthenticatedUser,
  ): Prisma.ProvisioningRequestWhereInput {
    if (user.role === Role.SUPER_ADMIN) return {};
    return {
      employeeId: { not: user.id },
      itemType: {
        approverType: ProvisioningApproverType.VERTICAL_OWNER,
        approverVertical: { ownerId: user.id },
      },
    };
  }

  private assertCanApprove(
    request: Awaited<ReturnType<ProvisioningService['findRequest']>>,
    user: AuthenticatedUser,
  ) {
    if (user.id === request.employeeId)
      throw new ForbiddenException(
        'You cannot approve your own provisioning request',
      );
    if (user.role === Role.SUPER_ADMIN) return;
    if (
      request.itemType.approverType ===
        ProvisioningApproverType.VERTICAL_OWNER &&
      request.itemType.approverVertical?.ownerId === user.id
    )
      return;
    throw new ForbiddenException(
      'Only the configured vertical owner or the CEO may act on this request',
    );
  }
  ...
  private async assertScm(user: AuthenticatedUser) {
    ...
    if (employee?.vertical?.code !== 'SCM')
      throw new ForbiddenException(
        'Only SCM users may fulfill provisioning requests',
      );
  }
  ...
  private assertSuperAdmin(user: AuthenticatedUser) {
    if (user.role !== Role.SUPER_ADMIN)
      throw new ForbiddenException(
        'Only the CEO may manage provisioning item types',
      );
  }

  private async normalizeItemType(
    dto:
      | CreateProvisioningItemTypeDto
      | (UpdateProvisioningItemTypeDto & {
          approverType: ProvisioningApproverType;
          name: string;
          requiresScmFulfillment: boolean;
        }),
  ) {
    const name = dto.name.trim();
    if (!name) throw new BadRequestException('Name is required');
    if (dto.approverType === ProvisioningApproverType.VERTICAL_OWNER) {
      if (!dto.approverVerticalId)
        throw new BadRequestException('An approver vertical is required');
      const vertical = await this.prisma.vertical.findUnique({
        where: { id: dto.approverVerticalId },
      });
      if (!vertical)
        throw new BadRequestException('Approver vertical not found');
      return {
        name,
        requiresScmFulfillment: dto.requiresScmFulfillment!,
        approverType: dto.approverType,
        approverVerticalId: dto.approverVerticalId,
      };
    }
    return {
      name,
