...
describe('FilingsService controls', () => {
  ...
  const service = new FilingsService(
    prisma as any,
    access as any,
    storage as any,
  );
  ...
  it('prevents a Finance Head from approving a GST return they prepared', async () => {
    prisma.gstReturn.findUnique.mockResolvedValue({
      id: 'g1',
      status: ComplianceReturnStatus.PENDING_APPROVAL,
      preparedById: head.id,
    });
    await expect(service.approveGst('g1', head)).rejects.toThrow(
      'cannot approve a GST return they prepared',
    );
  });

  it('prevents a Finance Head from approving a TDS return they prepared', async () => {
    prisma.tdsReturn.findUnique.mockResolvedValue({
      id: 't1',
      status: ComplianceReturnStatus.PENDING_APPROVAL,
      preparedById: head.id,
    });
    await expect(service.approveTds('t1', head)).rejects.toThrow(
      'cannot approve a TDS return they prepared',
    );
  });
  ...
  it('deduplicates imported GSTR-2B lines', async () => {
    ...
    await expect(
      service.importGstr2b({ taxPeriod: '2026-04', lines: [row, row] }, {
        id: 'user-1',
      } as any),
  ...
  it('calculates invoice TDS from the classified taxable base', async () => {
    ...
    const result: any = await service.setInvoiceTds(
      'ap-1',
      { sectionCode: '194C', ratePercent: 2, taxableBase: 12500 },
      { id: 'user-1' } as any,
    );
  ...
  it('prevents challan allocation beyond the deposited total', async () => {
    prisma.tdsChallan.findUnique.mockResolvedValue({
      id: 'c-1',
      totalAmount: '1000',
      allocations: [{ amount: '800' }],
    });
    prisma.tdsReturn.findUnique.mockResolvedValue({
      id: 't-1',
      status: 'PREPARED',
    ...
    await expect(
      service.allocateChallan('c-1', { tdsReturnId: 't-1', amount: 250 }, {
        id: 'user-1',
      } as any),
    ).rejects.toThrow('exceeds unallocated challan balance');
  });
});
