...
export class CloseControlsService {
  ...
  private async arOutstanding(to: Date) { const x = await this.prisma.salesInvoice.aggregate({ where: { invoiceDate: { lte: to }, status: { in: ['ISSUED','E_INVOICE_GENERATED','PARTIALLY_PAID','OVERDUE'] } }, _sum: { outstandingAmount: true } }); return this.d(x._sum.outstandingAmount); }
  ...
  private async outputGst(from: Date, to: Date) { const x = await this.prisma.salesInvoice.aggregate({ where: { invoiceDate: { gte: from, lte: to }, status: { in: ['ISSUED','E_INVOICE_GENERATED','PARTIALLY_PAID','PAID','OVERDUE'] } }, _sum: { cgstAmount: true, sgstAmount: true, igstAmount: true } }); return this.d(x._sum.cgstAmount).plus(this.d(x._sum.sgstAmount)).plus(this.d(x._sum.igstAmount)); }
