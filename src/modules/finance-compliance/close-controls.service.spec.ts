...
describe('CloseControlsService', () => {
  ...
  const prisma = {
    periodClose: { findUnique: jest.fn() },
    periodCloseTask: { findFirst: jest.fn(), update: jest.fn() },
  ...
  it('passes a reconciliation inside the one-paisa tolerance', () => {
    const result = (service as any).result(
      'TEST',
      'Test',
      new Prisma.Decimal('100.00'),
      new Prisma.Decimal('99.995'),
  ...
  it('allows only the Finance Head to waive an exception', async () => {
    prisma.reconciliationException.findUnique.mockResolvedValue({
      id: 'e-1',
      assignedToId: null,
      run: { periodClose: { status: 'PREPARING' } },
    });
    prisma.reconciliationException.update.mockResolvedValue({
      id: 'e-1',
      status: 'WAIVED',
    });
    await service.resolve(
      'e-1',
      { status: 'WAIVED', resolutionNote: 'Reviewed and immaterial' },
      { id: 'head-1' } as any,
    );
