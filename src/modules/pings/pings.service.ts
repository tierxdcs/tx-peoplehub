...
export class PingsService {
  ...
  async create(user: AuthenticatedUser, dto: CreatePingDto) {
    ...
    if (!recipientIds.length)
      throw new BadRequestException('Select at least one other recipient');
    const count = await this.prisma.employee.count({
      where: { id: { in: recipientIds }, status: EmployeeStatus.ACTIVE },
    });
    if (count !== recipientIds.length)
      throw new BadRequestException('One or more recipients are unavailable');
    const ping = await this.prisma.ping.create({
      data: {
        ...
        recipients: {
          create: recipientIds.map((employeeId) => ({ employeeId })),
        },
      },
      include: {
  ...
  async createContextual(
    ...
    dto: CreateContextualPingDto,
  ) {
    if (!dto.linkedRecordType || !dto.linkedRecordId)
      throw new BadRequestException(
        'A contextual ping must be linked to its current page',
      );
    const eligible = await this.recipients(user);
    const allowed = new Set(eligible.map((employee) => employee.id));
    if (dto.recipientIds.some((id) => !allowed.has(id)))
      throw new ForbiddenException(
        'One or more recipients do not have access to this page context',
      );
    return this.create(user, dto);
  }

  async received(user: AuthenticatedUser) {
    const rows = await this.withSchemaReady(() =>
      this.prisma.pingRecipient.findMany({
        where: { employeeId: user.id },
        include: {
          ping: { include: { fromEmployee: { select: employeeSelect } } },
        },
      }),
    );
    return rows
      ...
      .map((r) => ({
        ...r,
        ping: { ...r.ping, fromEmployee: toPingEmployee(r.ping.fromEmployee) },
      }));
  }
  ...
  async updateStatus(
    ...
    if (status === PingRecipientStatus.PENDING)
      throw new BadRequestException(
        'A response must acknowledge or resolve the ping',
      );
    const recipient = await this.prisma.pingRecipient.findUnique({
      where: { id },
    });
    if (!recipient) throw new NotFoundException('Ping recipient not found');
    if (recipient.employeeId !== user.id)
      throw new ForbiddenException('You can only respond to your own pings');
    if (recipient.status === PingRecipientStatus.RESOLVED)
      throw new BadRequestException('This ping is already resolved');
    if (
      recipient.status === PingRecipientStatus.ACKNOWLEDGED &&
  ...
  private async withSchemaReady<T>(operation: () => Promise<T>): Promise<T> {
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'Pings database migration is not applied. Run prisma migrate deploy and restart the API.',
        );
      }
      throw error;
    }
