/**
 * Rupee formatting and amount-in-words using the INDIAN numbering system
 * (Thousand → Lakh (10^5) → Crore (10^7)), NOT the Western
 * thousand/million/billion grouping.
 *
 * Port of web/app/lib/indian-number-words.ts + the `formatINR` grouping in
 * web/app/lib/sales.ts, for documents the backend renders itself (the emailed
 * Purchase Order PDF). Kept as pure functions over strings/numbers so the
 * document renderers stay unit-testable.
 *
 * Correctness note: a Western number-to-words library would render 389764 as
 * "Three Hundred Eighty-Nine Thousand …" — WRONG here. The Indian system groups
 * the leading digits in pairs, giving "Three Lakh Eighty-Nine Thousand Seven
 * Hundred Sixty-Four".
 */
...
/**
 * Whole-number → Indian-system words. 0 → "Zero". Handles arbitrary magnitudes
 * by prepending crore groups (…, Thousand Crore, Lakh Crore, Crore).
 */
 ...
 * Accepts a number or a decimal string (Prisma Decimals arrive as strings).
 */
...
/**
 * Lakh/crore-grouped amount with two decimals, e.g. "1,23,456.00". No currency
 * symbol: the documents label their own columns "(INR)", and the rupee glyph is
 * a font-availability gamble in a PDF.
 */
export function formatIndianAmount(value: string | number): string {
  const n = typeof value === 'number' ? value : Number(value);
