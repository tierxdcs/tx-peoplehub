...
export function PingWidget() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button size="icon" className="fixed bottom-5 right-5 z-40 size-14 rounded-full shadow-lg" aria-label="Send a contextual ping"><Radio className="size-6" /></Button></DialogTrigger><DialogContent className="max-w-md"><DialogHeader><DialogTitle>Ping someone</DialogTitle><DialogDescription>Linked automatically to {context.label.toLowerCase()}. {context.verticalCode ? `Recipients are scoped to ${context.verticalCode}.` : 'Recipients are scoped to people with access to this page.'}</DialogDescription></DialogHeader>
    <div className="max-h-56 space-y-1 overflow-y-auto rounded-md border p-2">{loading && people.length === 0 ? <p className="p-2 text-sm text-muted-foreground">Loading recipients…</p> : people.length === 0 ? <p className="p-2 text-sm text-muted-foreground">No eligible recipients on this page.</p> : people.map((person) => <label key={person.id} className="flex min-h-11 cursor-pointer items-center gap-3 rounded px-2 hover:bg-accent"><Checkbox checked={selected.includes(person.id)} onCheckedChange={(checked) => setSelected((old) => checked ? [...old, person.id] : old.filter((id) => id !== person.id))} /><span className="min-w-0"><span className="block truncate text-sm font-medium">{person.fullName}</span><span className="block truncate text-xs text-muted-foreground">{person.email}</span></span></label>)}</div>
