...
/**
 * My Leave content. Rendered as its own page (/leave) and as a tab in the
 * profile page. When `embedded`, drops PageContainer and moves the "Request
 * Leave" action into an inline bar (the host renders the section title).
 */
export function LeaveSection({ embedded = false }: { embedded?: boolean }) {
  ...
  const body = (
    <>
      {/* Embedded: no PageHeader, so surface the action in its own bar. */}
      {embedded && <div className="mb-4 flex justify-end">{requestLeaveButton}</div>}
  ...
  return (
    <PageContainer>
      <PageHeader title="My Leave" action={requestLeaveButton} />
      {body}
    </PageContainer>
  );
}
