'use client';

import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '../../../../components/ui/table';
import { FormEvent, useEffect, useState } from 'react';
import Link from 'next/link';
import { apiFetch, ApiError } from '../../../../lib/api';
import { useFinanceAccess } from '../../../../lib/use-finance-access';
import { Button } from '../../../../components/ui/button';
import { Input } from '../../../../components/ui/input';
import { Select } from '../../../../components/ui/select';
import { SCard, SignalHeader, SignalPage } from '../../../../components/ui/signal';
import { useToast } from '../../../../components/ui/toaster';
import { formatINR } from '../../../../lib/sales';
import { useNumberFormat } from '../../../../lib/number-format-context';
import { RegisterPagination } from '../../../../components/ui/register-pagination';
import { serverPageCount } from '../../../../lib/server-pagination';

interface Customer {
  id: string;
  name: string;
}

interface Invoice {
  id: string;
  invoiceNumber: string;
  customerId: string;
  outstandingAmount: string;
  status: string;
}

interface Receipt {
  id: string;
  receiptNumber: string;
  receiptDate: string;
  amount: string;
  unappliedAmount: string;
  status: string;
  customer: { name: string };
}

interface Page<T> {
  items: T[];
  total: number;
}

const PAGE_SIZE = 25;

export default function ReceiptsPage() {
  const toast = useToast(),
    { isAccountsHead } = useFinanceAccess();
  const { style: numberFormatStyle } = useNumberFormat();
  const [customers, setCustomers] = useState<Customer[]>([]),
    [invoices, setInvoices] = useState<Invoice[]>([]),
    [receipts, setReceipts] = useState<Receipt[]>([]);
  const [page, setPage] = useState(1),
    [total, setTotal] = useState(0);
  const [customerId, setCustomerId] = useState(''),
    [invoiceId, setInvoiceId] = useState(''),
    [amount, setAmount] = useState(''),
    [reference, setReference] = useState(''),
    [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const load = () =>
    Promise.all([
      apiFetch<Customer[]>('/finance/ar/reference/customers'),
      apiFetch<Page<Invoice>>('/finance/ar/invoices?limit=100'),
      apiFetch<Page<Receipt>>(`/finance/ar/receipts?page=${page}&limit=${PAGE_SIZE}`),
    ]).then(([c, i, r]) => {
      setCustomers(c);
      setInvoices(i.items);
      setReceipts(r.items);
      setTotal(r.total);
      if (!customerId && c[0]) setCustomerId(c[0].id);
    });
  useEffect(() => {
    load().catch((e) => toast.error(e instanceof ApiError ? e.message : 'Failed to load receipts'));
  }, [page]); // eslint-disable-line react-hooks/exhaustive-deps

  const open = invoices.filter((i) => i.customerId === customerId && ['ISSUED', 'PARTIALLY_PAID', 'OVERDUE'].includes(i.status));

  async function create(e: FormEvent) {
    e.preventDefault();
    try {
      await apiFetch('/finance/ar/receipts', {
        method: 'POST',
        body: JSON.stringify({
          customerId,
          receiptDate: date,
          currencyCode: 'INR',
          amount: Number(amount),
          paymentMethod: 'BANK_TRANSFER',
          bankReference: reference,
          allocations: invoiceId ? [{ invoiceId, amount: Number(amount) }] : [],
        }),
      });
      setAmount('');
      setReference('');
      toast.success('Draft receipt created');
      await load();
    } catch (x) {
      toast.error(x instanceof ApiError ? x.message : 'Failed');
    }
  }

  async function action(id: string, a: string) {
    try {
      await apiFetch(`/finance/ar/receipts/${id}/${a}`, { method: 'POST' });
      await load();
    } catch (x) {
      toast.error(x instanceof ApiError ? x.message : 'Failed');
    }
  }

  return (
    <SignalPage>
      <SignalHeader
        title="Customer Receipts"
        description="Allocate receipts to invoices or retain unapplied customer advances"
        actions={
          <Link href="/finance/vouchers/receipt/new">
            <Button variant="outline">New Receipt Voucher</Button>
          </Link>
        }
      />
      <div className="space-y-4 px-5 pb-7 pt-[18px] lg:px-7">
        <SCard className="px-5 py-[18px]">
          <form onSubmit={create} className="grid gap-3 md:grid-cols-6">
            <Select
              value={customerId}
              onChange={(e) => {
                setCustomerId(e.target.value);
                setInvoiceId('');
              }}
            >
              {customers.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name}
                </option>
              ))}
            </Select>
            <Select value={invoiceId} onChange={(e) => setInvoiceId(e.target.value)}>
              <option value="">Unapplied advance</option>
              {open.map((i) => (
                <option key={i.id} value={i.id}>
                  {i.invoiceNumber} · {formatINR(i.outstandingAmount, numberFormatStyle)}
                </option>
              ))}
            </Select>
            <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} />
            <Input required type="number" step="0.01" placeholder="Amount" value={amount} onChange={(e) => setAmount(e.target.value)} />
            <Input required placeholder="UTR / bank reference" value={reference} onChange={(e) => setReference(e.target.value)} />
            <Button type="submit">Create receipt</Button>
          </form>
        </SCard>
        <SCard className="overflow-hidden">
          <Table className="w-full text-sm">
            <TableHeader>
              <TableRow className="border-b text-left">
                <TableHead className="p-3">Receipt</TableHead>
                <TableHead>Customer</TableHead>
                <TableHead>Amount</TableHead>
                <TableHead>Unapplied</TableHead>
                <TableHead>Status</TableHead>
                <TableHead>Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {receipts.map((r) => (
                <TableRow className="border-b" key={r.id}>
                  <TableCell className="p-3 font-medium tabular-nums">{r.receiptNumber}</TableCell>
                  <TableCell>{r.customer.name}</TableCell>
                  <TableCell className="tabular-nums">{formatINR(r.amount, numberFormatStyle)}</TableCell>
                  <TableCell className="tabular-nums">{formatINR(r.unappliedAmount, numberFormatStyle)}</TableCell>
                  <TableCell>{r.status}</TableCell>
                  <TableCell>
                    {(r.status === 'DRAFT' || r.status === 'REJECTED') && (
                      <Button size="sm" variant="outline" onClick={() => action(r.id, 'submit')}>
                        Submit
                      </Button>
                    )}
                    {isAccountsHead && r.status === 'PENDING_APPROVAL' && (
                      <Button size="sm" onClick={() => action(r.id, 'approve')}>
                        Approve
                      </Button>
                    )}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        </SCard>
        <RegisterPagination page={page} pageCount={serverPageCount(total, PAGE_SIZE)} onPageChange={setPage} />
      </div>
    </SignalPage>
  );
}
