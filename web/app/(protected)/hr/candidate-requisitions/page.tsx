...
export default function CandidateRequisitionsPage() {
  ...
  const [positionTitle, setPositionTitle] = useState(''); const [employmentType, setEmploymentType] = useState('FULL_TIME'); const [justification, setJustification] = useState(''); const [budgetAnnualCtc, setBudgetAnnualCtc] = useState(''); const [targetJoiningDate, setTargetJoiningDate] = useState('');
  ...
  return <PageContainer><PageHeader title="Candidate Requisitions" description="Hiring authorization requires the vertical owner’s approval first, followed by CEO approval." />
    ...
    <RequisitionTable title={isSuperAdmin ? 'Awaiting final CEO approval' : 'Awaiting your vertical-owner approval'} items={queue} numberFormatStyle={numberFormatStyle} actions={(r) => <><Button size="sm" onClick={() => decide(r, true)}>Approve</Button><Button size="sm" variant="destructive" onClick={() => decide(r, false)}>Reject</Button></>} />
    {['MANAGER', 'ADMIN', 'SUPER_ADMIN'].includes(user?.role ?? '') && <RequisitionTable title="My requisitions" items={mine} numberFormatStyle={numberFormatStyle} />}
  </PageContainer>;
}
