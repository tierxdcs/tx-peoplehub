...
/**
 * Create + finalize a vendor audit (Internal Auditor/SA). Ten weighted score
 * inputs with a LIVE total + classification preview (same 90/80/70 thresholds
 * as the backend); the server re-verifies on submit. Finalizing sets the
 * vendor's status to the computed classification.
 */
  ...
  async function submit() {
    ...
    try {
      const payload: CreateAuditInput = {
        ...
        qualitySystemScore: Number(scores.qualitySystemScore),
        engineeringScore: Number(scores.engineeringScore),
        financialStabilityScore: Number(scores.financialStabilityScore),
        supplyChainScore: Number(scores.supplyChainScore),
        exportReadinessScore: Number(scores.exportReadinessScore),
        sustainabilityScore: Number(scores.sustainabilityScore),
        ehsScore: Number(scores.ehsScore),
        customerReferencesScore: Number(scores.customerReferencesScore),
      };
      await createAudit(vendorId, payload);
