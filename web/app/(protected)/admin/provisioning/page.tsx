...
export default function ProvisioningSettingsPage() {
  ...
  return <PageContainer><PageHeader title="Onboarding Provisioning" description="Configure which requests are created when a new hire receives ERP access, who approves them, and whether SCM fulfills them." />
    <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Item</TableHead><TableHead>Fulfillment</TableHead><TableHead>Approver</TableHead><TableHead>Active</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{items.map((i) => <TableRow key={i.id}><TableCell className="font-medium">{i.name}</TableCell><TableCell>{i.requiresScmFulfillment ? 'SCM (physical)' : 'Direct action'}</TableCell><TableCell>{i.approverType === 'SUPER_ADMIN' ? 'CEO' : `${i.approverVertical?.name ?? 'Unknown'} owner${i.approverVertical?.ownerId ? '' : ' (falls back to CEO)'}`}</TableCell><TableCell><Switch checked={i.isActive} onCheckedChange={() => toggle(i)} aria-label={`Toggle ${i.name}`} /></TableCell><TableCell className="text-right"><Button size="sm" variant="outline" onClick={() => edit(i)}>Edit</Button></TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
    <Card><CardHeader><CardTitle>{editingId ? 'Edit item type' : 'Add item type'}</CardTitle></CardHeader><CardContent><form onSubmit={save} className="grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">Name<Input className="mt-1" value={name} onChange={(e) => setName(e.target.value)} required /></label><label className="text-sm font-medium">Approval route<Select className="mt-1" value={approverType} onChange={(e) => setApproverType(e.target.value as typeof approverType)}><option value="SUPER_ADMIN">CEO</option><option value="VERTICAL_OWNER">Vertical owner</option></Select></label>{approverType === 'VERTICAL_OWNER' && <label className="text-sm font-medium">Approver vertical<Select className="mt-1" value={verticalId} onChange={(e) => setVerticalId(e.target.value)} required><option value="">Select vertical</option>{verticals.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</Select></label>}<label className="flex items-center gap-3 text-sm font-medium"><Switch checked={physical} onCheckedChange={setPhysical} /> Requires SCM fulfillment</label><div className="flex gap-2 md:col-span-2"><Button type="submit">{editingId ? 'Save changes' : 'Add item type'}</Button>{editingId && <Button type="button" variant="outline" onClick={resetForm}>Cancel</Button>}</div></form></CardContent></Card>
  </PageContainer>;
}
