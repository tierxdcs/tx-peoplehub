...
**Quote PDF filed to Vault on submit:** submitting renders the quote server-side (pdfkit) and files it into the **"RFQ Quotes"** DEFAULT Vault folder — SCM-scoped with an ACCOUNTS read grant, versioning on (5 retained), provisioned by the seed. Filename `RFQ-2026-0007_Vigyanlabs-Innovations_Quote.pdf`; a resubmission by the same partner adds a **new version** of that same file rather than a second file. The uploader recorded is the RFQ's creator (the vendor has no Employee row). Filing failures are logged and never fail the vendor's submission. The vendor's own uploaded attachments are **not** copied into Vault — they stay as `RfqQuote.attachmentFileKeys` object refs.
